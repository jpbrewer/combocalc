...
| `calc-modal.js` | UI CONTROLLER | On `DOMContentLoaded` | `window._comboCalc.closeModal` | DOM: modal overlay/panel/close + door bore chooser (single: #door-bore-left/#door-bore-right; double: #door-bore-left-double/#door-bore-right-double/#double_door_no_bore + #double-door-note/#double-door-note-dummy) + hardware color selector + muntin toggle + modal data grid; Data: reads `window._comboCalc` utilities, `window.comboSolutions`, `window.build_assembly_svg` | DOM: opens/closes modal, populates modal data grid, toggles muntins/bore/hardware color, Configure button (stages/reconciles/POSTs solution to Xano); Data: updates `solution.operating_door` (single: left_hand/right_hand; double: left_hand/right_hand/none), `solution.hardware_color` | Requires `calc-combo-results.js` (for `window._comboCalc`); `build_assembly_svg` must exist before Explore |
...
- **v4 (current)**
  - Added double-door operating_door controls in calc-modal.js (left_hand/right_hand/none toggle).
  - Added double-door hardware to SVG pipeline: bore circles on meeting stiles (`data-bore-slab`),
    slide bolts at top of swinging slab (`data-bolt-slab`), ball catches in head jamb (`data-catch`).
  - Extended `updateBoreVisibility` to control bore, bolt, and catch visibility for double doors.
  - Extended `updateHingeColor` to include bolt and catch rects.
